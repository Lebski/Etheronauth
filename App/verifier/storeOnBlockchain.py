...
def store_signature(permission_id, signature):
    ...
    permission_id_bytes = Web3.toBytes(text=permission_id)
    signature_bytes = Web3.toBytes(text=signature)
    deployed_Contract.functions.storeSignature(permission_id_bytes, signature_bytes).transact({'from': web3.eth.coinbase})
