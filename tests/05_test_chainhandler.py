...
request_id = chainhandler.submit_request(account, wait=False)
...
log.out.info("JSON-Token: {}".format(token))

log.out.info("# Checking store_signature")
token = chainhandler.store_signature(account, request_id, "TESTSIGNATURE", wait=False)
log.out.info("Reading again...")
token = chainhandler.request_token(account, request_id)
log.out.info("Signature from Token: {}".format(str(token["signature"])[0:30]))
